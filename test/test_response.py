...
async def test_download_response(test_cli):
    ...
    assert response_json == {"status": 200, "status": "pending", "total": 2}
...
async def test_download_response_already(test_cli):
    ...
    assert response.status == 200
    response_json = await response.json()
    assert response_json == {"status": 200, "status": "already", "total": 2}
...
async def test_download_zip_response(test_cli):
    ...
    assert response.status == 200
    response_json = await response.json()
    assert response_json == {"status": 200, "status": "pending"}
